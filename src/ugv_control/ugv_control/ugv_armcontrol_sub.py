...
server_ip   = self.get_parameter('server_ip').get_parameter_value().string_value or '0.0.0.0'
server_port = int(self.get_parameter('server_port').get_parameter_value().integer_value or 12345)
...
class UgvArmControlSubNode(Node):
    def __init__(self):
        ...
        server_ip   = get_typed_param(self, 'server_ip', '0.0.0.0') #change this
        server_port = get_typed_param(self, 'server_port', 12345)
        self.client_ip   = get_typed_param(self, 'client_ip', '127.0.0.1') #change this
   ...
   # --- man_ctrl callback ---
    def on_man_ctrl(self, msg: ManCtrl):
        ...
        try:
            ...
            self.get_logger().debug(f'Sent MAN {arm_cmd=}')
        except Exception as ex:
    ...
    def on_auto_ctrl(self, msg: AutoCtrl):
        ...
        try:
            ...
            self.get_logger().debug(
                f'Sent ARM_CMD={self.ARM_CMD}, flag={auto_flag}, err={heading_error:.3f}'
            )
        except Exception as ex:
