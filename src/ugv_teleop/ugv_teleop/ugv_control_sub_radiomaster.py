...
class UgvControlSubRadiomasterNode(Node):
    def __init__(self):
        super().__init__('ugv_control_sub_radiomaster')
        ...
        self.get_logger().info('UGV RADIOMASTER SUBSCRIBER STARTED')

    def _heartbeat_listener(self):
        while rclpy.ok():
            try:
                self.sock.settimeout(1.0)
                _, addr = self.sock.recvfrom(256)
                with self._hb_lock:
    ...
    def on_man_ctrl(self, msg: ManCtrl):
        ...
        self._send(arm_payload, 'RM ARM', self.arm_ip, self.arm_port)
        self._send(drive_payload, 'RM DRIVE', self.drive_ip, self.drive_port)
    ...
    def on_auto_ctrl(self, msg: AutoCtrl):
        ...
        self._send(payload, 'RM AUTO DRIVE', self.drive_ip, self.drive_port)
    ...
    def destroy_node(self):
        self.get_logger().info('Shutting down ugv_control_sub_radiomaster...')
        try:
...
def main():
    rclpy.init()
    node = UgvControlSubRadiomasterNode()
    try:
        rclpy.spin(node)
