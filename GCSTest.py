...
def _handle_sigint(sig, frame):
    global _stop
    _stop = True
    print("\n[GCS] Stop requested — finishing current cycle...")
...
def ContinuousTest():
    ...
    while not _stop:
        try:
            seq += 1
            print(f"[GCS → JETSON] Sending heartbeat #{seq}...")
            SendCommand(Heartbeat(ConnectionStatus.Connected), Vehicle.MRA)

            TelemetryInstance = ReceiveTelemetry()

            if TelemetryInstance is not None:
                print(f"[JETSON → GCS] Hearing you! {TelemetryInstance}")
            else:
                print(f"[GCS] No telemetry on cycle #{seq}")
    ...
    print("[GCS] Continuous test stopped.")

if __name__ == '__main__':
    while True:
        ContinuousTest()
        Restart = input("\n[GCS] Restart? (y/n): ").strip().lower()
        if Restart != "y":
            break